...
export default function SidebarConversations() {
    ...
    const updateConversations = () => {
        if (currentUser) {
            const conversations = currentUser.conversations.map((conversation: any) => conversation.members.filter((member: any) => (member._id !== currentUser._id) && (member.username.toLowerCase().includes(searchInput.toLowerCase()))))
            dispatch(addToConversationArray(conversations[0]))
        }
    }
